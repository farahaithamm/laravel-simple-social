        ...
        <div class="col-md-4 mb30 mt-30" style="padding-top: 20px">
            <div class="card">
                <div class="card-content pt20 pb20 profile-header">
                    <img src="{{ asset(auth()->user()->pfp ?? 'uploads/defaultpfp.png') }}"
                    alt="Profile Picture"
                    class="d-block mx-auto"
                    style="width: 100px; height: 100px; object-fit: cover; border-radius: 50%;">

                    <h2 class="text-center mb10" >{{auth()->user()->name}}</h2>
                    <h5 class="text-center mb20" style="color:#999">{{auth()->user()->email}}</h5>
                    <p>
                        {{auth()->user()->bio}}
                    </p>
                    <li class="list-inline-item g-mr-20">
                        <a class="u-link-v5 g-color-gray-dark-v4 g-color-primary--hover" href="{{route('user.edit', auth()->id())}}">
                            Edit
                        </a>
                    </li>
                    <li class="list-inline-item g-mr-20">
                        <form action="{{ route('user.destroy', auth()->id())}}" method="POST" onsubmit="return confirm('Are you sure you want to delete your profile?');">
                            @csrf
                            @method('DELETE')
